<template>
	<div class="flex text-right justify-items-end justify-end mt-2">
		<button
			class="vfm-btn bg-slate-200 px-4 py-2 text-xs"
			@click="$emit('closeModal')"
		>
			공유<br />버튼
		</button>
		<button
			class="vfm-btn bg-slate-200 px-4 py-2 text-xs ml-2"
			@click="$emit('closeModal')"
		>
			모임 전용 <br />
			메뉴
	...
	<div class="border mt-5 flex p-5 items-center justify-between">
		<div class="flex items-center">
			<div
				class="inline-block h-12 w-12 rounded-full ring-2 ring-slate-200 bg-slate-200"
			/>
	...
	<div class="text-center">
		<BoardMain />
		<button
			class="border px-5 py-2 mt-5"
			@click="router.push('meeting/detail')"
		>
			게시글 더보기
		</button>
		<VoteMain />
